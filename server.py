...
def get_reminders():
 ...
 'uid': uid,
 'summary': summary,
 'due': due.dt.isoformat() if due else None,
...
MCP_TOOLS = [
 {
 "name": "get_events",
 "description": "Get calendar events for a specific date range. Defaults to today if no parameters given.",
 "inputSchema": {
 "type": "object",
 "properties": {
 "days_from_now": {
 "type": "integer",
 "description": "Days offset from today (0=today, 1=tomorrow, -1=yesterday). Default 0."
 },
 "days_count": {
 "type": "integer",
 ...
 "days_from_now": {
 "type": "integer",
 "description": "Start date offset. Default 0."
 },
 "days_count": {
 "type": "integer",
 "description": "Search range in days. Default 30."
 }
 },
 "required": ["keyword"]
 }
 ...
 "uid": {
 "type": "string",
 "description": "Event UID (get from get_events or search_events)"
 }
 },
 "required": ["uid"]
 }
 ...
 {
 "name": "get_reminders",
 "description": "Get all active reminders (todos) from iCloud.",
 "inputSchema": {
 "type": "object",
 "properties": {}
 }
...
def handle_tool_call(tool_name, arguments):
 ...
 days_count = arguments.get('days_count', 1)
 events = get_events_by_range(days_from, days_count)
 return {
 "content": [
 {
 "type": "text",
 ...
 {
 "type": "text",
 "text": json.dumps(reminders, indent=2, ensure_ascii=False)
 }
 ]
 }
...
def main():
 ...
 print(f"Endpoint: http://localhost:{MCP_PORT}/mcp")
